...
class Logger:
    ...
    def __new__(cls, 
                logger_name: str = None, 
                   ...
                   print_log = False,
                   level = None):
        os.makedirs(DEAULT_LOG_DIR, exist_ok=True)
    ...
    def add_logger(logger_name: str, 
        ...
        if file_name is not None:
            if os.path.splitext(file_name)[-1] == '':
                file_path = osp.join(DEAULT_LOG_DIR, f'{file_name}.{file_format}')
            else:
                file_path = osp.join(DEAULT_LOG_DIR, file_name)
        else:
            file_path = osp.join(DEAULT_LOG_DIR, f'{logger_name}.{file_format}')
